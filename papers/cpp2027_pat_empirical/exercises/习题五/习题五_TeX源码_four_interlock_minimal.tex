...
PatFourInterlockMinimal.lean & DOI \\
\end{longtable}
}
\end{quote}
...
\begin{quote}
习题定位：课后习题系列第五题（三体习题的延续------用户洞察的直接形式化）。用户指出：三互锁无法达成
⟹ 无限外推理论在 n = 3 处存在漏洞（无法从单互锁自然增加到三互锁）⟹
\textbf{四互锁是最小自洽结构}，需要证明；且四互锁如果再收敛，就会被自指吸收。本题完成该证明。全部新定理只锚框架定理（R136/R140/R147/R149/R154/R134），不用外部引理。
\end{quote}

\emph{2026-08-13 · Internal research exercise · Lean 4 / mathlib v4.32.2
· 7 new theorems PROVED no sorry · 课后习题 V }
...
+2（2→4→6→\ldots）。这是对互锁结构的一个结构修正（用户洞察，PROVED
于本习题）。
...
三体不可积性 & 三互锁断裂的经典对应 \\
\textbf{R136 ②③}（） & 方向必须成对声明 & 互锁成对的元规则 \\
\textbf{R149}（） & 4 相位两两互锁 & 四互锁自洽的锚定 \\
\textbf{R154}（） & S³ 无损内收 & 再收敛 = 自指的几何机制 \\
\textbf{R134}（） & pat0 吸收一切操作 & 自指吸收的锚定 \\
\end{longtable}
}

\begin{center}\rule{0.5\linewidth}{0.5pt}\end{center}

\emph{课后习题 V · 2026-08-13 · Lean 4 / mathlib v4.32.2 · 7
