...
only so `transport` covariance can be proven inside the class. In an abelian heap
...
class Heap (H : Type*) where
  ...
  It is kept here as a field so `transport` covariance is provable in the
...
/-- The basepoint-change transport `T_{e→f}(x) = [x,e,f]` is a heap automorphism
(in any heap). -/
theorem transport_is_aut {H : Type*} [Heap H] (e f : H) :
